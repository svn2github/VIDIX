...
int main( void )
{
  ...
  {
    printf(" Bus:card:func vend:dev  command  base0   :base1   :base2   :baserom :irq:pin:gnt:lat\n");
    for(i=0;i<num_pci;i++)
#if __WORDSIZE > 32
	printf("%04X:%04X:%04X %04X:%04X %04X %16X:%16X:%16X:%16X:%02X :%02X :%02X :%02X\n"
#else
	printf("%04X:%04X:%04X %04X:%04X %04X %08X:%08X:%08X:%08X:%02X :%02X :%02X :%02X\n"
#endif
